...
export default function HeroSection({ id, name, title, tagline, imageUrl, imageHint, socialLinks, cvUrl }: HeroSectionProps) {
  return (
    <section
      id={id}
      className="min-h-[calc(100vh-4rem)] md:min-h-[calc(100vh-5rem)] flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 text-center bg-primary text-primary-foreground"
    >
      <div className="container mx-auto max-w-screen-lg flex flex-col md:flex-row items-center gap-8 md:gap-12">
        <div className="md:order-2 flex-shrink-0">
          <Image
            ...
            width={240}
            height={240}
            className="w-48 h-48 md:w-60 md:h-60 rounded-full mx-auto shadow-xl border-4 border-primary-foreground/50 object-cover"
            priority
          />
        </div>
