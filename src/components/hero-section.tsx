...
export default function HeroSection({ id, name, title, tagline, imageUrl, imageHint, socialLinks, cvUrl }: HeroSectionProps) {
  return (
    <section
      id={id}
      className="min-h-[calc(100vh-4rem)] md:min-h-[calc(100vh-5rem)] flex items-center justify-center py-16 px-4 sm:px-6 lg:px-8 text-center bg-background text-foreground"
    >
      <div className="container mx-auto max-w-screen-lg flex flex-col md:flex-row items-center gap-8 md:gap-12">
        ...
        <div className="md:order-1 md:text-left text-center">
          ...
          <p className="text-lg sm:text-xl md:text-2xl text-foreground/90 mb-4">{title}</p>
          <p className="text-md sm:text-lg text-foreground/75 max-w-xl mx-auto md:mx-0 mb-8">{tagline}</p>
          <div className="flex flex-col sm:flex-row justify-center md:justify-start items-center gap-4">
            ...
            <div className="flex space-x-3 mt-4 sm:mt-0">
              <Button asChild variant="outline" size="icon" className="rounded-full bg-transparent border-primary text-primary hover:bg-primary hover:text-primary-foreground">
